...
def test_train_data():
    train_set = make_dataloader(train_path)
...
def test_test_data():
    test_set = make_dataloader(test_path)
    assert len() == N_train/batch_size, "Test Data: dataset did not have the correct number of samples"
    for tweets, att_mask, labels in test_set:
        assert tweets.size(dim=0)==batch_size and tweets.size(dim=1)==72, "Test Data: Size of tweets attributes are incorrect"
        assert att_mask.size(dim=0)==batch_size and att_mask.size(dim=1)==72, "Test Data: Size of att_mask attribute is incorrect"
